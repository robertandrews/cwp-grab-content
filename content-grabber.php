...
function content_grabber_admin_page()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $posts_data = content_grabber_handle_api_response($api_response_body);
        }
         */
        if (!is_wp_error($content_url)) {
            // content_grabber_director($content_url);

            if (strstr($content_url, 'wp-json')) {
                // WP-JSON URL
                echo "That's a WP-JSON URL";
                // grab_from_json($content_url);
                // TODO: Fetch the JSON to $post_data
            } elseif (strstr($content_url, 'feed')) {
                // RSS URL
                echo "That's an RSS feed URL";
                // grab_from_rss($content_url);
            } else {
                // Normal URL
                // echo "normal URL";
                // $created_post_id = grab_from_web($content_url);
                if ($created_post_id) {
...
// TODO: Not this
    ...
    if (isset($posts_data) && is_array($posts_data)) {
        ...
        echo '<th scope="col" id="title" class="manage-column column-title column-primary">Title</th>';
        echo '</tr>';
        echo '</thead>';
        ...
        foreach ($posts_data as $post_data) {
            ...
            echo '<td class="title column-title has-row-actions column-primary"><strong><a href="' . $post_data['link'] . '">' . $post_data['title']['rendered'] . '</a></strong></td>';
            echo '</tr>';
        }
        echo '</tbody>';

        echo '</table>';
        echo '<button type="button" class="button button-primary">Import Selected Posts</button>';
    }
    ?>
...
function content_grabber_director($content_url)
{
    /**
...
function grab_from_web($content_url)
{
    ...
    // file_put_contents('body.txt', print_r($response, true));
    ...
    if ($wp_post_api_url = wp_find_post_api_url($body)) {
        ...
        // print_r("WordPress discovered.");
        // echo $wp_post_api_url;
        ...
        $posts_json = wp_call_api_posts($post_type, $post_id, $content_url);
        // print_r($posts_json);
    ...
    if ($post_package) {
        ...
        // TODO: If something else re JSON
        if ($created_post_id) {
...
function wp_call_api_posts($post_type, $post_id, $content_url)
{
